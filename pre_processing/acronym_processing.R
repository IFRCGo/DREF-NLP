...
subdimension$Set[sample(nrow(subdimension), 300)] <- 1
...
subdimension_rest$Set[sample(nrow(subdimension_rest), 300)] <- 1
...
abbs <- read.csv(paste0(getwd(), "/data/abbreviations.csv"), sep = ";")


subdimension_test <- subdimension

for (i in 1:dim(abbs)[1]) {
  
  acronym1 <- paste0(" ",  abbs[i, 1], " ")
  word <- paste0(" ", abbs[i, 2], " ")

  subdimension <- subdimension%>% 
    mutate(Excerpt =gsub(acronym1, word, Excerpt))  
  
  acronym2 <- paste0("(",  abbs[i, 1], ")")
  ...
  for (item in punctuation) {
    ...
    acronym3 <- paste0(abbs[i, 1], item)
    
    subdimension <- subdimension%>% 
      mutate(Excerpt =gsub(acronym3, word, Excerpt))  
...
id <- rownames(subdimension)
subdimension_id <- cbind(id=id, subdimension)


subdimension_test <- subdimension_id %>% 
  unnest_tokens(input = Excerpt, output = word) %>%
  filter(!word %in% stop_words$word) %>% 
  group_by(id) %>%
  summarise(text = paste(word, collapse = " ")) %>% 
  inner_join(subdimension_id, by="id") %>%
  ungroup() %>% 
  select(-id, 
         -Excerpt) %>% 
  rename(Excerpt=text)
...
write.table(subdimension_test, file = paste0(getwd(), "/data/training/training_data_dref.csv"), sep = "\t" , fileEncoding = "UTF-8")
...
words <-c("run", "ran", "running")
new_words <- lemmatize_words(words)

subdimension_test2 <- subdimension_id %>% 
  unnest_tokens(input = Excerpt, output = word) %>%
...
write.table(subdimension_test2, file = paste0(getwd(), "/data/training/training_data_dref.csv"), sep = "\t" , fileEncoding = "UTF-8")
